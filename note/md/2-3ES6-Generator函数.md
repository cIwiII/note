...
- Generator 函数是 ES6 提供的一种异步编程解决方案，英语中，generator 这个词是“生成器”的意思

- 语法上，Generator 函数除了状态机，还是一个遍历器对象生成函数。可以依次遍历 Generator 函数内部的每一个状态。

- 形式上，是一个普通函数，但有两个特征。
  - 一是，`function`关键字与函数名之间有一个星号；
  - 二是，函数体内部使用`yield`（“产出”）表达式，定义不同的内部状态。
...
// 调用不会执行，仅是一个指向内部状态的指针，指向函数头部，hw就是一个遍历器对象
var hw = helloWorldGenerator(); 
```

- Generator 函数`helloWorldGenerator`，有三个状态：hello，world 和 return 语句

- 每次调用`next`方法，内部指针就从函数头部或上一次停下来的地方开始执行，直到遇到`yield`表达式（或`return`语句）为止。
- 换言之，Generator 函数是分段执行的，`yield`表达式是暂停执行的标记，而`next`方法可以恢复执行。

```javascript
hw.next()
// { value: 'hello', done: false }  // 这是 next 方法返回的对象
...
hw.next()
// { value: undefined, done: true } //已经结束，之后再调用都是这个值，没有return也是这个
```
...
function * foo(x, y) { ··· }
function *foo(x, y) { ··· }
function* foo(x, y) { ··· }// Generator 函数仍然是普通函数,一般是这种
function*foo(x, y) { ··· }
```
...
遍历器对象`只有`调用`next`方法才会遍历下一个内部状态。`yield`表达式就是暂停标志，只能用在 Generator 函数中，用于其他地方都会报错。

`next`方法的运行逻辑：

- 遇到`yield 或 return` 就将后面表达式的值返回给value属性，到函数结束都没有，就返回undefined作为value属性值

```javascript
function* gen() { // 初始指针在此处
  yield  123 + 456; // next 执行后，指针在这个表达式末尾
}
```
...
var generator = f();// 不会执行，暂缓
setTimeout(function () {
  generator.next() // 没有yield，一个next执行所有代码
}, 2000);
```
...
var flat = function* (a) {
  a.forEach(function (item) {
    if (typeof item !== 'number') {
      yield* flat(item); // 报错，forEach的回调函数是普通函数
    } else {
      yield item; // 报错，forEach的回调函数是普通函数
    }
  });
};
...
任意一个对象的`Symbol.iterator`方法，等于该对象的遍历器生成函数，调用该函数会返回该对象的一个遍历器对象。Generator 函数就是遍历器生成函数，所以可以赋值给`Symbol.iterator`属性，从而使得该对象具有 Iterator 接口。
...
[...myIterable] // [1, 2, 3] //... 是遍历，遍历调用的是next方法
```

Generator 函数执行后，返回一个遍历器对象。该遍历器对象本身也具有`Symbol.iterator`属性，执行后返回自身。
...
`yield`表达式本身没有返回值(总是 undefined )，`next`方法可以带一个参数当作上一个`yield`表达式的返回值。
...
g.next(true) // { value: 0, done: false } // 只要传入为真，i被赋值为1，执行i++，为0，永远返回这个，
```

这个功能的意义在于Generator 函数只是 `暂停或恢复`，它的上下文状态（context）是不变的。通过`next`方法的参数，从外部向内部注入不同的值，从而调整函数行为。
...
var a = foo(5);
a.next() // Object{value:6, done:false}  // 返回 x + 1 , x 为 5
a.next() // Object{value:NaN, done:false}  // next恢复时没有传入值，为undefined，y/为undefined=NaN
a.next() // Object{value:NaN, done:true}   // 2*undefined 为NaN，return 3/NaN 也是NaN

var b = foo(5);
b.next() // { value:6, done:false }
b.next(12) // { value:8, done:false } // 12代替 x+1, 24/3 返回8
b.next(13) // { value:42, done:true } // 13 代替 y/3，return 5 + (2*12) + 13 = 42
```
...
function* dataConsumer() {
  console.log("Started");
  console.log(`1. ${yield}`); // 暂停，根据第二个next的传入值决定
  console.log(`2. ${yield}`); // 暂停，根据第三个next的传入值决定
  return "result";
}
...
wrapped().next('hello!') //本质就是在内部(第三行) 调用了一次，此处这里就是第二次了
// First input: hello!
```
...
- next 方法done为true，就停止且不包含done为true部分，所以没6
- 由于return 返回已经执行，即使再手动调用next，value也是undefined
...
let jane = { first: 'Jane', last: 'Doe' }; // 不具备迭代接口，无法for of
