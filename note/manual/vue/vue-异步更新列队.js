...
class Reactive {
  ...
  createObserver() {
    ...
    return {
      ...
      // 用来标志当前应用是否有更新任务 默认 false 没有更新任务
      isUpdate: false, // 任务队列，存放是待更新的状态值
      queue: new Set(), // 异步执行的更新方法
      update() {
        // key 就是我们的状态属性名
      ...
      emit(key) {
        ...
        // 当前应用启动了更新任务
        this.isUpdate = true; // 要延迟进行视图的更新，异步队列
  ...
  defineProperties() {
    // 响应式对象
    this.proxy = {}; // vue 2.x 中使用 Object.defineProperty
...
var vm = new Reactive({
  el: '#app',
  data() {
    return { msg: 'hello world', count: 1, a: 1 };
  }
});
